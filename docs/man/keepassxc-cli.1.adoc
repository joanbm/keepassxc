...
*ssh-agent-populate* <__database__> [_entry_]::
  Adds keys to the SSH Agent, and removes them when a quit signal is received.
  If no entry is specified, defaults to adding all keys specified to load on database open.
